...
export default function CharacterList({
  ...
  const handleNext = () => {
    if (
      ...
      selectedCharacterIndex < filteredCharacters.length - 1
    ) {
  ...
  const selectedCharacter =
    selectedCharacterIndex !== null
      ? filteredCharacters[selectedCharacterIndex]
      : null;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-400">
            Comprehensive guide for all Umamusume characters. Click on a
            character to view their detailed guide.
          </p>
        </div>
              ...
              >
                <div className="aspect-[3/4] relative flex items-center justify-center p-2 bg-neutral-800">
                  ...
                  <Image
                    src={`/${character.image}`}
                    alt={character.name}
                    className="w-[115px] h-[115px] group-hover:w-32 group-hover:h-32 object-contain transition-all duration-300 z-0"
                    width={115}
                    height={115}
                    loading="lazy"
                  />
                  <div className="absolute bottom-0 left-0 right-0 p-3 z-20">
                    <h3 className="text-sm font-medium text-white truncate text-center group-hover:text-pink-400 transition-colors">
                      {character.name}
                    </h3>
                  </div>
                </div>
