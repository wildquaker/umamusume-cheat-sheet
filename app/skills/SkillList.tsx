...
export default function SkillList({ skills }: { skills: Skill[] }) {
  ...
  const filteredSkills = skills.filter((skill) => {
    const matchesSearch = skill.Skill.toLowerCase().includes(
      searchTerm.toLowerCase(),
    );
    const matchesRating =
      ratingFilter === "All" || skill.Rating === ratingFilter;
    const matchesStyle = styleFilter === "All" || skill.Style === styleFilter;
    const matchesDistance =
      distanceFilter === "All" || skill.Distance === distanceFilter;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-400">
            Recommended skills categorized by rating, style, and distance.
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-4">
        <div className="flex items-center gap-2">
          <label htmlFor="rating-filter" className="text-sm text-neutral-400">
            Rating:
          </label>
          <select
            id="rating-filter"
            ...
            className="bg-neutral-900 border border-neutral-800 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-pink-500/50"
          >
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="style-filter" className="text-sm text-neutral-400">
            Style:
          </label>
          <select
            id="style-filter"
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="distance-filter" className="text-sm text-neutral-400">
            Distance:
          </label>
          <select
            id="distance-filter"
        ...
        <button
          ...
          className="flex items-center gap-2 bg-neutral-800 hover:bg-neutral-700 border border-neutral-700 rounded-lg px-3 py-2 text-neutral-300 hover:text-white text-sm transition-colors focus:outline-none focus:border-pink-500/50"
        >
          <svg
