...
import type { Scenario, DistanceGroup, Build } from "../lib/types";
      ...
      <div className="space-y-8">
        <div className="prose prose-invert max-w-none">
          <p className="text-neutral-400">
            Recommended support card decks and target stats for different
            scenarios and distances.
          </p>
        </div>

        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
          {buildsData.map((scenario: Scenario, sIdx: number) => (
            <div key={sIdx} className="space-y-6">
              ...
              <div className="grid gap-6">
                {scenario.builds.map(
                  (distanceGroup: DistanceGroup, dIdx: number) => (
                    <div
                      key={dIdx}
                    ...
                    >
                      ...
                      <div className="divide-y divide-neutral-800">
                        {distanceGroup.build.map(
                          (build: Build, bIdx: number) => (
                            <div key={bIdx} className="p-4 space-y-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {build.Cards.map(
                                  (card: string, cIdx: number) => (
                                    <div
                                      key={cIdx}
                                      className="w-[66px] h-[66px] bg-neutral-800 rounded-lg overflow-hidden border border-neutral-700 relative group shrink-0"
                                    >
                                      {cardImages[card] ? (
                                        <Image
                                          src={cardImages[card]}
                              ...
                              <div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 text-sm">
                                  {Object.entries(build["Target Stats"]).map(
                                    ([stat, value]) => {
                                      if (!value) return null;
                                      return (
                                        <div
                                          key={stat}
                                          className="bg-neutral-800/50 rounded p-2 text-center border border-neutral-800"
                                        >
